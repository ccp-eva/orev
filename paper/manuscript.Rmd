...
orev_stab <- read_csv("../../vocab/data/clean_data.csv")%>%
  ...
  left_join(read_csv("../data/stability_data.csv")%>%
              group_by(subjID, test_day)%>%
              rename(test_day_t2 = test_day)%>%
              summarise(orev_t2 = sum(correct)))%>%
  na.omit()%>%
  ungroup()%>%
